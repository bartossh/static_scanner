use super::inspection::{Evidence, Mark};
...
///
pub trait Scanner {
    fn scan(&self, tokens: &PreTokenizedString) -> Option<Evidence>;
}
...
impl Scanner for Json {
    fn scan(&self, suspected_crime: &PreTokenizedString) -> Option<Evidence> {
        ...
        //Some(Evidence::new(&self.name, values))
        None
    }
}
...
impl Presenter for Fingerprint {
    fn name(&self) -> String {
        self.name.clone()
    }
}
